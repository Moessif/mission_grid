...
- `rosbridge_port`: rosbridge 端口
- `web_video_port`: web_video_server 端口
- `mavlink_port`: MAVLink 端口

**配置文件**: `mission_grid/config.json`（自动生成，不提交 Git）

**主要函数**:
- `load_config()`: 加载配置
- `save_config(config)`: 保存配置

**被引用**: main_window, remote_service, video_stream, point_cloud

---

### network_scanner.py — 网络扫描

**职责**: 扫描局域网中的 OrangePi 设备。

**主要类**:
- `NetworkScanner`: 扫描线程

**扫描方式**: 使用 nmap 快速扫描 SSH 端口（22）

**信号**:
- `device_found(ip, hostname)`: 发现设备
- `scan_progress(msg)`: 扫描进度
- `scan_finished()`: 扫描完成
